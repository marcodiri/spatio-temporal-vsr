...
class SRNet(nn.Module):
    ...
    def __init__(self, in_nc=3, out_nc=3, nf=64, nb=16, upsample_func=None, scale=4):
...
class SpatioTemporalDiscriminator(BaseDiscriminator):
    """Spatio-Temporal discriminator in proposed in TecoGAN"""

    def __init__(self, in_nc=3, *, spatial_size, tempo_range=3, scale=4):
    ...
    def forward(self, data, args_dict):
        """
        :param data: should be either hr_fake or hr_true
        ...
        # ------------ classify ------------ #
        pred = self.forward_single(input_data)  # out, feature_list
